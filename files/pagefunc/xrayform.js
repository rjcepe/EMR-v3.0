...
//////////////////////////////// sort function
  ...
  async function loadTableData() {
    ...
    const { data: tableData1, error } = await _supabase.from("xray_forms").select("*");
    ...
    const tableBody = document.querySelector("#xray_table tbody");
...
document.getElementById("searchpatient").addEventListener("click", async function() {
    ...
    const { data: patientData, error } = await _supabase.from("xray_forms").select("*").eq("patient_id", searchID);
